...
import { n as buildMihomoYaml, r as buildUriList, t as buildB64Subscription } from "./mihomo-BZFtDaS-.mjs";
import { t as runScan } from "./scan.server-DqwgDPZX.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/github-EgQNMFbE.js
...
function pickExportNodes(result, limit) {
	const alive = result.nodes.filter((n) => n.alive);
	const pool = result.probeMode === "mihomo" ? alive : alive.length ? alive : result.nodes;
...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/router-P2QMY_Oy.js
var router_P2QMY_Oy_exports = /* @__PURE__ */ __exportAll({ getRouter: () => getRouter });
...
var styles_default = "/assets/styles-D2byU3Se.css";
var APP_NAME = "Relay";
...
var $$splitComponentImporter = () => import("./routes-CAM8uBxm.mjs");
...
var Route = createFileRoute("/api/sub")({ server: { handlers: {
	...
	GET: async ({ request }) => {
		...
		const fmtRaw = (url.searchParams.get("fmt") ?? "b64").toLowerCase();
		const fmt = fmtRaw === "clash" || fmtRaw === "uri" ? fmtRaw : "b64";
		const limit = Math.min(80, Math.max(4, Number(url.searchParams.get("n") || 40) || 40));
		const real = url.searchParams.get("real") !== "0";
		const testUrl = url.searchParams.get("test") || void 0;
		...
		headers.set("x-relay-alive", String(result.nodes.filter((n) => n.alive).length));
		if (fmt === "uri") {
...
//#endregion
export { normalizeSourceUrl as a, sourceNameFromUrl as c, getRouter, formatMs as i, cn as n, pickActive as o, encodeSourceParam as r, pickExportNodes as s, router_P2QMY_Oy_exports as t };
