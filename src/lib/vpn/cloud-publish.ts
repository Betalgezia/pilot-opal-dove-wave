...
function previewUrl(settings: CloudPublishSettings, fmt: "b64" | "clash"): string {
  return `https://gist.githubusercontent.com/${encodeURIComponent(settings.username.trim())}/${settings.gistId.trim()}/raw/${fmt === "clash" ? "relay.yaml" : "relay.b64"}`;
}
...
async function publishOne(settings: CloudPublishSettings, subscription: string, format: "b64" | "clash", liveCount: number): Promise<CloudPublishResult> {
  ...
  try {
    response = await fetch(`https://api.github.com/gists/${encodeURIComponent(id)}`, {
      ...
      body: JSON.stringify({ files: { [filename]: { content: subscription }, "published.at": { content: new Date(at).toISOString() }, "live.count": { content: String(liveCount) } } }),
    });
...
async function recordResult(settings: CloudPublishSettings, result: CloudPublishResult, kind: "ok" | "warn"): Promise<void> {
  await writeLog(kind === "ok" ? "info" : "warn", kind === "ok" ? `[PUBLISH] gists/${settings.gistId.trim()} → ${result.status} ok, ${result.live} live, ${formatBytes(result.bytes)}` : `[PUBLISH] failed: ${result.error ?? "ошибка публикации"}`, { gistId: settings.gistId.trim(), username: settings.username.trim(), fmt: result.fmt, status: result.status, bytes: result.bytes, live: result.live, url: result.url, at: result.at });
}

export async function publishToGist(args: CloudPublishSettings & { subscription: string; format?: "b64" | "clash"; liveCount?: number }): Promise<CloudPublishResult> {
  return publishOne(args, args.subscription, args.format ?? "b64", args.liveCount ?? 0);
}
...
async function publishFormats(loadPayload: (fmt: "b64" | "clash") => Promise<{ subscription: string; live: number }>, settings: CloudPublishSettings): Promise<CloudPublishResult[]> {
  ...
  for (const fmt of ["b64", "clash"] as const) {
    ...
    try {
      const payload = await loadPayload(fmt);
      bytes = new TextEncoder().encode(payload.subscription).byteLength;
      live = payload.live;
      const result = await publishToGist({ ...settings, subscription: payload.subscription, format: fmt, liveCount: live });
      const item = { ...result, at: startedAt };
      results.push(item);
      await recordResult(settings, item, "ok");
    } catch (error) {
      ...
      results.push(item);
      await recordResult(settings, item, "warn");
    }
  }
  writeStatus(results);
  return results;
...
export async function publishScanToGist(result: ScanResult, settings = loadCloudPublishSettings()): Promise<CloudPublishResult[]> {
  return publishFormats((fmt) => payloadFromScan(result, fmt), settings);
}

export async function publishCurrentToGist(settings = loadCloudPublishSettings()): Promise<CloudPublishResult[]> {
  return publishFormats(payloadFromCurrent, settings);
}

export function CloudPublishController(): null {
  return null;
}
