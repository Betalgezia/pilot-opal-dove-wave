...
const NAV_GROUPS: Array<{ title: string; items: Array<{ id: ViewId; label: string; icon: typeof LayoutDashboard }> }> = [
  ...
  },
  {
    title: "Данные",
    items: [
      { id: "sources", label: "Источники", icon: Globe2 },
      { id: "pool", label: "Пул", icon: Database },
    ],
  },
  {
    title: "Управление",
...
export function Dashboard() {
  ...
  useEffect(() => {
    void pollProgress();
    const id = window.setInterval(() => void pollProgress(), 800);
  ...
  const scan = useMutation({
    mutationFn: async () => {
      ...
      return scanSources({
        data: {
          sources: enabled,
          perSource: Math.max(100, settings.perSource),
          globalCap: Math.max(100, settings.globalCap),
          timeoutMs: Math.max(500, settings.timeoutMs),
  ...
  async function stopScan() {
    ...
    try {
      ...
      toast.error("Не удалось остановить сканирование");
    }
  }
    ...
    <div className="min-h-dvh min-w-0 overflow-x-hidden bg-bg">
      <header className="sticky top-0 z-30 border-b border-border/70 bg-bg/90 backdrop-blur-xl">
        <div className="mx-auto flex min-h-16 w-full max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-border bg-surface shadow-border"><Radio className="size-5 text-primary" /></span>
            <div className="min-w-0"><p className="font-display text-sm font-semibold tracking-tight">Relay</p><p className="truncate text-[11px] text-fg-muted">network subscription control plane</p></div>
          </div>
          <div className="flex items-center gap-2">
            <span className="hidden items-center gap-2 rounded-full border border-border bg-surface px-3 py-1.5 text-[10px] font-mono text-fg-muted sm:flex"><span className={`size-1.5 rounded-full ${statusTone}`} />{statusLabel}</span>
            <Badge variant={mihomoOk ? "live" : "warn"}>mihomo {mihomoOk ? "ready" : "off"}</Badge>
            <Dialog><DialogTrigger asChild><Button variant="ghost" size="icon" aria-label="Справка"><Info /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Relay</DialogTitle><DialogDescription>Relay собирает источники, проверяет узлы, формирует пул и отдаёт постоянную live-подписку. Панель рассчитана на эксплуатацию и диагностику, а не только на выдачу ссылки.</DialogDescription></DialogHeader></DialogContent></Dialog>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto grid w-full max-w-[1440px] gap-5 px-4 pb-16 pt-5 sm:px-6 lg:grid-cols-[14.5rem_minmax(0,1fr)] lg:px-8">
        <aside className="lg:sticky lg:top-[84px] lg:self-start">
          <nav className="rounded-2xl border border-border bg-surface p-2 shadow-border">
          ...
          <section className={`mt-3 rounded-2xl border p-3 shadow-border transition-colors ${progress.active || settings.autoRefresh ? "border-primary/30 bg-primary/[0.035]" : "border-border bg-surface"}`}>
            <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold">Control</p><p className="mt-0.5 text-[10px] text-fg-muted">{progress.active ? progress.message : settings.autoRefresh ? `auto · ${settings.refreshMinutes} min` : "manual"}</p></div><Activity className={`size-4 ${progress.active || settings.autoRefresh ? "animate-pulse text-primary" : "text-fg-subtle"}`} /></div>
            <div className="mt-3"><div className="flex items-center justify-between text-[9px] uppercase tracking-[0.12em] text-fg-subtle"><span>{progress.active ? progress.phase : progress.phase === "done" ? "ready" : progress.phase}</span><span className="font-mono">{Math.round(progress.percent)}%</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-bg-subtle"><div className={`h-full rounded-full transition-all duration-500 ${progress.phase === "done" ? "bg-live" : "bg-primary"}`} style={{ width: `${progress.percent}%` }} /></div></div>
            <Button className="mt-3 h-9 w-full text-xs" onClick={() => progress.active || scan.isPending ? void stopScan() : startScan()}><span className="flex items-center gap-2">{progress.active || scan.isPending ? <Square className="size-3.5 fill-current" /> : <RefreshCcw className="size-3.5" />}{progress.active || scan.isPending ? "Остановить" : "Сканировать"}</span></Button>
            <button type="button" onClick={cycleStrategy} className="mt-2 flex w-full items-center justify-between rounded-lg bg-bg-subtle px-2.5 py-2 text-[10px] text-fg-muted"><span>Стратегия</span><span className="font-mono text-fg">{strategyLabel}</span></button>
          </section>
        </aside>
...
function PanelFrame({ icon, title, description, actions, children }: { icon: React.ReactNode; title: string; description: string; actions?: React.ReactNode; children: React.ReactNode }) {
...
function HomeView({ result, sources, total, alive, dead, unknown, parsedTotal, healthySources, livePercent, statusLabel, progress, setView, logsUnread, strategyLabel }: { result: ScanResult | null; sources: SourceDef[]; total: number; alive: number; dead: number; unknown: number; parsedTotal: number; healthySources: number; livePercent: number; statusLabel: string; progress: ScanProgress; setView: (view: ViewId) => void; logsUnread: number; strategyLabel: string }) {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[0.16em] text-fg-subtle">Control plane</p><h1 className="mt-1 text-xl font-semibold tracking-tight">Главная</h1><p className="mt-1 text-xs text-fg-muted">Состояние источников, пула и live-подписки в одном месте.</p></div><Badge variant={statusLabel === "READY" ? "live" : statusLabel === "ERROR" ? "dead" : statusLabel === "SCANNING" ? "warn" : "default"}>{statusLabel}</Badge></div>
    <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
      <MetricTile icon={<Globe2 />} label="Источники" value={sources.length} meta={`${healthySources}/${sources.length || 0} healthy`} />
      <MetricTile icon={<Zap />} label="URI" value={parsedTotal} meta={`${result?.uniqueTotal ?? 0} unique`} />
      <MetricTile icon={<Wifi />} label="LIVE" value={alive} tone="live" meta={`${dead} dead · ${unknown} unknown`} />
      <div className="rounded-2xl border border-border bg-surface p-4 shadow-border"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-[10px] font-medium uppercase tracking-wider text-fg-subtle"><Gauge className="size-3.5 text-live" />READY</span><span className="font-mono text-sm text-live">{livePercent}%</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-bg-subtle"><div className="h-full rounded-full bg-live transition-all duration-500" style={{ width: `${livePercent}%` }} /></div><div className="mt-2 flex items-center justify-between text-[10px] text-fg-muted"><span>{alive} / {total} nodes</span><span>{poolTone === "live" ? "healthy pool" : poolTone === "warn" ? "partial health" : "needs attention"}</span></div></div>
    </div>

    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(20rem,0.8fr)]">
      <section className="rounded-2xl border border-border bg-surface p-4 shadow-border"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-lg bg-bg-subtle"><Globe2 className="size-4 text-primary" /></span><div><h2 className="text-sm font-semibold">Источник → состояние пула</h2><p className="text-[10px] text-fg-subtle">live / parsed по последнему скану</p></div></div><button type="button" onClick={() => setView("sources")} className="text-[10px] text-fg-muted hover:text-fg">Открыть источники →</button></div><div className="space-y-2">{(result?.sources ?? []).slice(0, 6).map((source) => { const percent = source.parsed ? Math.round((source.alive / source.parsed) * 100) : 0; return <div key={source.id} className="rounded-xl border border-border/70 bg-bg-subtle/50 px-3 py-2.5"><div className="flex items-center gap-3"><span className={`size-2 shrink-0 rounded-full ${source.ok && source.alive > 0 ? "bg-live" : source.ok ? "bg-warning" : "bg-danger"}`} /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><span className="truncate text-xs font-medium">{source.name}</span><span className="shrink-0 font-mono text-[10px] text-fg-muted">{source.alive} live / {source.parsed} URI</span></div><div className="mt-2 flex items-center gap-2"><div className="h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-bg"><div className="h-full rounded-full bg-live" style={{ width: `${Math.min(100, percent)}%` }} /></div><span className="w-8 shrink-0 text-right font-mono text-[9px] text-live">{percent}%</span></div></div></div></div>; })}{!result?.sources?.length ? <div className="rounded-xl border border-dashed border-border px-4 py-10 text-center text-xs text-fg-muted">Запусти сканирование, чтобы увидеть состояние источников.</div> : null}</div></section>
      <div className="space-y-4">
        <section className="rounded-2xl border border-border bg-surface p-4 shadow-border"><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-lg bg-bg-subtle"><Activity className="size-4 text-primary" /></span><div><h2 className="text-sm font-semibold">Runtime</h2><p className="text-[10px] text-fg-subtle">операционный контекст</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><RuntimeField label="Probe" value={result?.probeMode ?? "—"} mono /><RuntimeField label="Test URL" value={result?.testUrl ?? "—"} mono /><RuntimeField label="Last scan" value={result ? new Date(result.scannedAt).toLocaleTimeString() : "—"} mono /><RuntimeField label="Duration" value={result ? formatMs(result.durationMs) : "—"} mono /></div><div className="mt-3 flex items-center justify-between rounded-xl bg-bg-subtle px-3 py-2 text-[10px] text-fg-muted"><span>Strategy</span><span className="font-mono text-fg">{strategyLabel}</span></div></section>
        <section className="rounded-2xl border border-border bg-surface p-4 shadow-border"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-lg bg-bg-subtle"><FileText className="size-4 text-primary" /></span><div><h2 className="text-sm font-semibold">Signals</h2><p className="text-[10px] text-fg-subtle">за последние 10 минут</p></div></div><button type="button" onClick={() => setView("logs")} className="text-[10px] text-fg-muted hover:text-fg">Логи →</button></div><div className="mt-3 flex items-center justify-between rounded-xl bg-bg-subtle px-3 py-2.5"><span className="text-xs text-fg-muted">Ошибки</span><span className={`font-mono text-sm ${logsUnread ? "text-danger" : "text-live"}`}>{logsUnread}</span></div></section>
      </div>
    </div>
...
function StatisticsView({ result, sources, alive, total, parsedTotal }: { result: ScanResult | null; sources: SourceDef[]; alive: number; total: number; parsedTotal: number }) {
  ...
  return <div className="space-y-4">
    <PanelFrame icon={<BarChart3 className="size-4 text-primary" />} title="Статистика" description="Метрики последнего сканирования и качество пула" actions={result ? <Badge variant="live">{new Date(result.scannedAt).toLocaleTimeString()}</Badge> : <Badge>нет данных</Badge>}>
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><MetricTile icon={<Zap />} label="Parsed" value={parsedTotal} meta={`${sources.length} configured sources`} /><MetricTile icon={<Database />} label="Unique" value={result?.uniqueTotal ?? 0} meta={`${result?.nodes.length ?? 0} probed`} /><MetricTile icon={<Wifi />} label="Live rate" value={`${liveRate}%`} tone="live" meta={`${alive}/${total} nodes`} /><MetricTile icon={<Clock3 />} label="Scan" value={result ? formatMs(result.durationMs) : "—"} meta={result?.probeMode ?? "—"} /></div>
      <div className="grid gap-4 xl:grid-cols-2">
        <section className="rounded-2xl border border-border bg-bg-subtle/45 p-4"><div className="flex items-center gap-2"><Gauge className="size-4 text-primary" /><h2 className="text-sm font-semibold">Pipeline timings</h2></div><div className="mt-3 space-y-2">{metrics ? <><TimingRow label="Fetch" value={metrics.fetchMs} total={metrics.totalMs} /><TimingRow label="Parse" value={metrics.parseMs} total={metrics.totalMs} /><TimingRow label="Sample" value={metrics.sampleMs} total={metrics.totalMs} /><TimingRow label="Probe" value={metrics.probeMs} total={metrics.totalMs} /><TimingRow label="GeoIP" value={metrics.geoIpMs} total={metrics.totalMs} /><TimingRow label="Deep verify" value={metrics.deepVerifyMs} total={metrics.totalMs} /><TimingRow label="Quality" value={metrics.qualityMs} total={metrics.totalMs} /></> : <p className="text-xs text-fg-muted">Метрики появятся после следующего скана.</p>}</div></section>
        <section className="rounded-2xl border border-border bg-bg-subtle/45 p-4"><div className="flex items-center gap-2"><Server className="size-4 text-primary" /><h2 className="text-sm font-semibold">Probe counters</h2></div><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">{metrics ? <><StatBox label="Sampled" value={metrics.sampled} /><StatBox label="Deep verified" value={metrics.deepVerified} /><StatBox label="Target checks" value={metrics.targetChecks} /><StatBox label="mihomo loaded" value={metrics.mihomoLoaded} /><StatBox label="delay received" value={metrics.mihomoDelayReceived} /><StatBox label="unknown" value={metrics.unknown} /></> : <StatBox label="status" value="—" />}</div></section>
      </div>
      <section className="rounded-2xl border border-border bg-bg-subtle/45 p-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Globe2 className="size-4 text-primary" /><h2 className="text-sm font-semibold">Best source latency</h2></div><span className="font-mono text-xs text-live">{best ? formatMs(best.bestLatency) : "—"}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{(result?.sources ?? []).map((source) => <div key={source.id} className="flex items-center justify-between rounded-lg bg-bg px-3 py-2 text-[10px]"><span className="truncate text-fg-muted">{source.name}</span><span className="font-mono text-fg">{formatMs(source.bestLatency)}</span></div>)}</div></section>
    </PanelFrame>
  </div>;
}

function SettingsView({ settings, setSettings, mihomoOk, updateNumber }: { settings: Settings; setSettings: React.Dispatch<React.SetStateAction<Settings>>; mihomoOk: boolean; updateNumber: (key: keyof Pick<Settings, "refreshMinutes" | "perSource" | "globalCap" | "timeoutMs">, value: string) => void }) {
  return <div className="space-y-4">
    <PanelFrame icon={<Settings2 className="size-4 text-primary" />} title="Настройки" description="Параметры runtime, probe, scan и export">
      <div className="grid gap-4 xl:grid-cols-2">
        <SettingsCard icon={<RefreshCcw />} title="Scan" description="Периодичность и ограничители сканирования"><SettingToggle label="Автообновление" hint="автоматически запускать скан" checked={settings.autoRefresh} onChange={(autoRefresh) => setSettings((current) => ({ ...current, autoRefresh }))} /><NumberField label="Интервал, мин" value={settings.refreshMinutes} onChange={(value) => updateNumber("refreshMinutes", value)} min={1} max={60} /><div className="grid gap-3 sm:grid-cols-2"><NumberField label="URI / source" value={settings.perSource} onChange={(value) => updateNumber("perSource", value)} min={100} max={50000} /><NumberField label="Global cap" value={settings.globalCap} onChange={(value) => updateNumber("globalCap", value)} min={100} max={100000} /></div></SettingsCard>
        <SettingsCard icon={<ShieldCheck />} title="Probe" description="Проверка реальной доступности узлов"><SettingToggle label="Real probe" hint={mihomoOk ? "трафик через mihomo" : "mihomo недоступен"} checked={settings.realProbe && mihomoOk} disabled={!mihomoOk} onChange={(realProbe) => setSettings((current) => ({ ...current, realProbe }))} /><NumberField label="Timeout, ms" value={settings.timeoutMs} onChange={(value) => updateNumber("timeoutMs", value)} min={500} max={30000} /><label className="grid gap-1.5"><Label htmlFor="settings-test-url">Test URL</Label><Input id="settings-test-url" value={settings.testUrl} onChange={(event) => setSettings((current) => ({ ...current, testUrl: event.target.value }))} className="font-mono text-xs" /></label></SettingsCard>
        <SettingsCard icon={<Zap />} title="Selection" description="Как выбирается активная нода"><button type="button" onClick={() => { const order: SelectStrategy[] = ["fastest", "fallback", "balanced"]; const next = order[(order.indexOf(settings.strategy) + 1) % order.length]; setSettings((current) => ({ ...current, strategy: next })); }} className="flex w-full items-center justify-between rounded-xl bg-bg-subtle px-3 py-3 text-xs"><span>Стратегия</span><span className="font-mono text-fg">{settings.strategy}</span></button><div className="mt-2 text-[10px] text-fg-muted">{settings.strategy === "fastest" ? "Предпочитает минимальную задержку." : settings.strategy === "fallback" ? "Сначала удерживает работоспособный источник." : "Балансирует выбор между источниками."}</div></SettingsCard>
        <SettingsCard icon={<Download />} title="Export" description="Формат и размер выдачи"><div className="flex flex-wrap gap-2">{(["clash", "b64", "uri"] as ExportFormat[]).map((fmt) => <button key={fmt} type="button" onClick={() => setSettings((current) => ({ ...current, exportFmt: fmt }))} className={`rounded-lg border px-3 py-2 font-mono text-xs ${settings.exportFmt === fmt ? "border-primary bg-primary text-primary-foreground" : "border-border bg-bg-subtle text-fg-muted hover:text-fg"}`}>{fmt}</button>)}</div><div className="mt-3 flex flex-wrap gap-2">{[12, 24, 40, 60].map((count) => <button key={count} type="button" onClick={() => setSettings((current) => ({ ...current, exportN: count }))} className={`rounded-lg border px-3 py-2 font-mono text-xs ${settings.exportN === count ? "border-primary bg-primary text-primary-foreground" : "border-border bg-bg-subtle text-fg-muted hover:text-fg"}`}>{count} nodes</button>)}</div></SettingsCard>
      </div>
      <div className="flex items-center gap-2 rounded-xl border border-border bg-bg-subtle px-3 py-2.5 text-[10px] text-fg-muted"><Check className="size-3.5 text-live" />Настройки сохраняются в браузере и синхронизируются с live-профилем Relay.</div>
    </PanelFrame>
  </div>;
}

function MetricTile({ icon, label, value, meta, tone = "" }: { icon: React.ReactNode; label: string; value: number | string; meta?: string; tone?: "live" | "danger" | "warn" | "" }) {
  ...
  return <div className="rounded-2xl border border-border bg-surface p-4 shadow-border"><div className="flex items-start justify-between gap-3"><span className="flex size-8 items-center justify-center rounded-lg bg-bg-subtle">{typeof icon === "object" ? <span className={iconClass}>{icon}</span> : icon}</span><span className="text-[9px] uppercase tracking-wider text-fg-subtle">{label}</span></div><p className={`mt-3 font-mono text-2xl leading-none ${valueClass}`}>{value}</p>{meta ? <p className="mt-1.5 truncate text-[10px] text-fg-muted">{meta}</p> : null}</div>;
}
...
function SettingsCard({ icon, title, description, children }: { icon: React.ReactNode; title: string; description: string; children: React.ReactNode }) { return <section className="rounded-2xl border border-border bg-surface p-4 shadow-border"><div className="flex items-start gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-bg-subtle text-primary">{icon}</span><div><h2 className="text-sm font-semibold">{title}</h2><p className="mt-0.5 text-[10px] text-fg-subtle">{description}</p></div></div><div className="mt-4 space-y-3">{children}</div></section>; }
