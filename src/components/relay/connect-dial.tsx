...
type DialState = "idle" | "scanning" | "live" | "dead" | "stopped";
...
export function ConnectDial({
  ...
  const progress = latency === null ? 0 : Math.max(0.12, Math.min(1, 1 - latency / 420));
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={state === "scanning" ? "Остановить сканирование" : "Запустить сканирование"}
      className={cn(
    ...
    >
      <svg viewBox="0 0 120 120" className="absolute inset-3 text-border" aria-hidden>
        <circle cx="60" cy="60" r="52" fill="none" stroke="currentColor" strokeWidth="1.25" />
        <circle
      ...
      <div className="relative flex flex-col items-center gap-1 px-6 text-center">
        {state === "scanning" ? (
          <Square className="mb-1 size-6 fill-current text-fg-muted" />
        ) : (
          <span className={cn(
        ...
        {state === "live" && node ? (
          <>
            ...
            <span className="font-mono text-lg tabular-nums text-fg">{formatMs(latency)}</span>
          </>
        ) : (
