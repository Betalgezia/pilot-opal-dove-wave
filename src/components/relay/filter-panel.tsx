...
import { Check, RotateCcw, Save, ShieldBan, ShieldCheck, SlidersHorizontal } from "lucide-react";
...
export function FilterPanel({ result, testUrl, onTestUrl }: { result: ScanResult | null; testUrl: string; onTestUrl: (value: string) => void }) {
  ...
  const filtered = result?.nodes.filter((node) => node.alive).length ?? 0;

  function toggleProtocol(protocol: VpnProtocol) {
    setFilters((current) => ({
      ...current,
      protocols: current.protocols.includes(protocol) ? current.protocols.filter((item) => item !== protocol) : [...current.protocols, protocol],
    }));
  }
  ...
  function toggleCountry(country: string) {
    setFilters((current) => ({
      ...current,
      countries: current.countries.includes(country) ? current.countries.filter((item) => item !== country) : [...current.countries, country],
    }));
  }
  ...
  return (
    <div className="space-y-4">
      <PanelHeader icon={<SlidersHorizontal className="size-4 text-primary" />} title="Фильтрация подписки" description="Фильтры применяются к live-подписке и экспортируемому пулу." actions={<>
        <Button variant="ghost" size="sm" onClick={() => void reset()}><RotateCcw />Сброс</Button>
        <Button size="sm" onClick={() => void save()} disabled={!loaded}><Save />Сохранить</Button>
      </>} />
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,0.8fr)]">
        <section className="rounded-2xl border border-border bg-surface p-4 shadow-border">
          <div className="flex items-start justify-between gap-4">
            <div><div className="flex items-center gap-2"><ShieldCheck className="size-4 text-live" /><h2 className="text-sm font-semibold">Protocol filter</h2></div><p className="mt-1 text-xs text-fg-muted">Пустой список = разрешены все протоколы.</p></div>
            <Badge variant={filters.protocols.length ? "default" : "live"}>{filters.protocols.length ? `${filters.protocols.length} выбрано` : "all"}</Badge>
          </div>
          <div className="mt-4 flex flex-wrap gap-2">
            {PROTOCOL_OPTIONS.map((protocol) => {
              const selected = filters.protocols.includes(protocol);
              return <button key={protocol} type="button" onClick={() => toggleProtocol(protocol)} className={`rounded-lg border px-3 py-2 font-mono text-xs transition-colors ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border bg-bg-subtle text-fg-muted hover:text-fg"}`}>{selected ? <Check className="mr-1 inline size-3" /> : null}{PROTOCOL_LABELS[protocol]}</button>;
            })}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-border bg-surface p-4 shadow-border">
          <div className="flex items-start justify-between gap-4">
            <div><div className="flex items-center gap-2"><ShieldCheck className="size-4 text-live" /><h2 className="text-sm font-semibold">Country filter</h2></div><p className="mt-1 text-xs text-fg-muted">Ограничивает узлы по GeoIP.</p></div>
            <Badge>{filters.countryMode}</Badge>
          </div>
          <div className="mt-4 flex flex-wrap gap-2">
            {COUNTRY_MODES.map((mode) => <button key={mode.id} type="button" onClick={() => setFilters((current) => ({ ...current, countryMode: mode.id }))} className={`rounded-lg border px-3 py-2 text-xs ${filters.countryMode === mode.id ? "border-primary bg-primary text-primary-foreground" : "border-border bg-bg-subtle text-fg-muted hover:text-fg"}`}>{mode.label}</button>)}
          </div>
      ...
      <div className="flex items-center justify-between rounded-xl bg-bg-subtle px-4 py-3 text-[11px] text-fg-muted"><span>Текущий скан: {result ? `${result.uniqueTotal} unique → ${alive} live` : "нет результата"}</span><span>После фильтров live-выбор формируется при экспорте.</span><span className="hidden font-mono md:inline">{filtered} alive base</span></div>
    </div>
  );
}

function PanelHeader({ icon, title, description, actions }: { icon: React.ReactNode; title: string; description: string; actions: React.ReactNode }) {
